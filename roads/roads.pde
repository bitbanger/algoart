...
void setup() {
  ...
  try {
    ...
    BufferedReader reader = createReader("roads.pde");
    String line;
  ...
  size(int(dim*SCALE_FAC), int(dim*SCALE_FAC));
  background(0xFF, 0xFF, 0xFF);
  
  getImgColors("stripes.png");
  
  fill(0xFF, 0xFF, 0xFF);
  rect(0, 0, dim*SCALE_FAC, dim*SCALE_FAC);
...
class Paver {
  ...
  Paver(float x, float y, float theta, float vel, int life, int depth) {
    ...
    this.codePt = int(random(source.length()));
  ...
  void move() {
    if(life > 0) {
      ...
      if(hit[nx][ny] > 10000 || abs(hit[nx][ny]-theta) < 5) {
        ...
        fill(c, a);
        if(random(1.0) < 1.0/8) {
          text(source.charAt(codePt), x, y);
        }
        
        codePt = (codePt+1) % source.length();*/
